...
public class StepsDefinition {
  ...
  public void userIsNotAuthenticated() {
    accessToken = null;
    request = RestAssured.given();
    request.headers("Accept", "application/vnd.allegro.public.v1+json");
  }

  @When("^User calls GET IDs of Allegro categories$")
  public void userCallGETIDsOfAllegroCategories() {
    response = request.get("/sale/categories").then().extract().response();
  }
  ...
  public void userCallGETACategoryByIDWithIdValue(String idValue) {
    response = request.get("/sale/categories/" + idValue).then().extract().response();
  }
  ...
  public void userCallsGETParametersSupportedByACategoryWithIDValueIdValue(String idValue) {
    response =
        request.get("/sale/categories/" + idValue + "/parameters").then().extract().response();
  }
        ...
        (element) -> {
          ...
          response
              ...
              .body(
                  "categories",
                  hasItem(
                      allOf(allOf(hasEntry("id", categoryId), hasEntry("name", categoryName)))));
        });
  }
  ...
  public void receiveAmountOfCategories(String amount) {
    response.then().statusCode(200).body("categories", hasSize(Integer.parseInt(amount)));
  }
  ...
  public void userReceivesCategoryCategoryName(String categoryName) {
    response.then().statusCode(200).body("name", equalTo(categoryName));
  }

  @Then("^Receives (\\d+) parameters$")
  public void receivesParametersAmountParameters(String parametersAmount) {
    response.then().statusCode(200).body("parameters", hasSize(Integer.parseInt(parametersAmount)));
  }
  ...
  public void thereAreFollowingCategoriesCategories(String categoriesString) {
    ...
    for (String category : categories) {
      response
          .then()
          .statusCode(200)
          .body("parameters", hasItem(allOf(hasEntry("name", category))));
    }
  }
}
